...
MODEL_PATH = "v1.pt"  # Path to your YOLOv8 model
CONF_THRESHOLD = 0.6
IMGSZ = 640
DEVICE = None  # None (auto), "cpu", or "cuda:0"

POLL_INTERVAL = 0.2  # Poll every 200ms for maximum responsiveness
#MAX_CYCLES = 2  # Stop after processing 2 photos
POLL_TIMEOUT = 60  # Maximum time to wait for new photo (seconds)
...
def poll_for_new_photo(last_hash, timeout):
    ...
    while time.time() - start_time < timeout:
        poll_count += 1
        try:
            resp = requests.get(f"{RPI_URL}/get_photo", timeout=2)
            ...
            pass  # Connection error, keep trying
...
def save_annotated_frame(img_bgr, boxes, names, save_path, conf_threshold):
    ...
    for i in range(len(boxes)):
        ...
        # Draw label
        label = f"{obj_id}_{pretty}_{conf_i:.2f}"
...
def save_individual_clips(img_bgr, boxes, names, gallery_dir, conf_threshold):
    """Save individual cropped images for each detection."""
    saved_symbols = set()
...
def decide_command(detected_symbols):
    ...
    Priority: look for arrow directions (left/right).
    """
    ...
    for obj_id, pretty, conf in detected_symbols:
        ...
        print(f"[PC]    Detected: {pretty} (ID: {obj_id}, conf: {conf:.2f})")
        
        if pretty_lower == "left" or obj_id == 39:
            print(f"[PC]  Decision: LEFT arrow detected → sending L")
            return "l"
        elif pretty_lower == "right" or obj_id == 38:
            print(f"[PC] Decision: RIGHT arrow detected → sending R")
            return "r"
    
    # Default if no clear arrow found
    print("[PC]  No clear left/right arrow, defaulting to R")
    return "r"

def send_command_to_rpi(command):
    """Send l or r command back to RPI."""
    try:
        resp = requests.post(f"{RPI_URL}/send_command", data=command, timeout=5)
        if resp.status_code == 200:
            print(f"[PC] Sent command '{command}' to RPI")
            return True
        else:
...
def main():
    ...
    while True:
        ...
        if not success:
            print(f"[PC] No new photo detected or connection lost. Retrying...")
            time.sleep(1)

    
    # Create tiled gallery at the end
    print(f"\n[PC] Creating tiled gallery...")
